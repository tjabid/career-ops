...
- `location_exclude`: Location keywords to skip (e.g. "India", "Bangalore")
...
**Each company MUST have `careers_url` in portals.yml.** If it doesn't have one, find it once, save it, and use it in future scans.
...
5. **Level 2 — ATS APIs / feeds** (parallel):
   ...
   d. For **BambooHR**, the list only brings basic metadata. For each relevant item, read `id`, GET `https://{company}.bamboohr.com/careers/{id}/detail`, and extract the full JD from `result.jobOpening`. Use `jobOpeningShareUrl` as public URL if present; if not, use the detail URL.
...
6.5. **Filter by location** using `location_exclude` from `portals.yml`:
   - If the job's location field contains any `location_exclude` keyword (case-insensitive), skip it
   - Record as `skipped_location` in scan-history.tsv
...
7.5. **Verify liveness of Level 3 WebSearch results** — BEFORE adding to pipeline:
   ...
   WebSearch results may be outdated (Google caches results for weeks or months). To avoid evaluating expired offers, verify with Playwright each new URL from Level 3. Levels 1 and 2 are inherently real-time and do not require this verification.
...
11. **Location-excluded offers**: record with status `skipped_location`
...
https://...	2026-02-10	ashby-api	EM India	Zapier	skipped_location
```
